...
- **最新版更适合“官方媒体主线优先”**：图像、视频、TTS、音乐入口更清晰，旧 Skill 更适合补充而不是主线
- **如果你追最新版**：建议把 `infer` CLI 当成实验台，把 Agent 工具调用当成正式工作流，把 ComfyUI 当成本地编排层
- **如果你只按教程基线跑**：本章大多数创意思路仍能借鉴，但旧命令和旧 Skill 名称一定要先自行验证
- **本章后文看到整串 `clawhub install ...`**：请默认理解为“历史案例素材”，不要直接当作 `v2026.4.14` 默认安装清单
